...
class PacketSnifferApp:
    def __init__(self, master):
        ...
        self.title_label = tk.Label(master, text="Enhanced Packet Sniffer", font=("Helvetica", 20), bg="#f0f0f0")
        ...
        # Create a frame for buttons and filters
        self.control_frame = tk.Frame(master, bg="#f0f0f0")
        self.control_frame.pack(pady=5)

        # Create a start button
        self.start_button = tk.Button(self.control_frame, text="Start Capturing", command=self.start_capturing, bg="#4CAF50", fg="white", font=("Helvetica", 12))
        self.start_button.pack(side=tk.LEFT, padx=5)

        # Create a stop button
        self.stop_button = tk.Button(self.control_frame, text="Stop Capturing", command=self.stop_capturing, bg="#f44336", fg="white", font=("Helvetica", 12))
        self.stop_button.pack(side=tk.LEFT, padx=5)

        # Create a filter selection
        self.protocol_var = tk.StringVar(value="All")
        self.filter_label = tk.Label(self.control_frame, text="Filter by Protocol:", bg="#f0f0f0")
        self.filter_label.pack(side=tk.LEFT, padx=5)
        self.protocol_menu = tk.OptionMenu(self.control_frame, self.protocol_var, "All", "TCP", "UDP", "ICMP")
    ...
    def packet_callback(self, packet):
        ...
        # Extract protocol and filter based on selection
    ...
    def extract_packet_info(self, packet):
        ...
        if IP in packet:
            ip_layer = packet[IP]
            info += f"Source IP: {ip_layer.src} -> Destination IP: {ip_layer.dst}\n"
    ...
    def display_packet_info(self, packet_info, packet):
        ...
        # Color based on IPs (example; modify as needed)
        color_tag = "neutral"  # Default
